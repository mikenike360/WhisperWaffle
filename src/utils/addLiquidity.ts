...
export async function createPoolNativeAleo(
  ...
  try {
    ...
    if (!minLpTokens || isNaN(minLpTokens) || minLpTokens <= 0) {
      throw new Error('Invalid minimum LP tokens');
    }
    ...
    const createPoolInputs = [
      ...
      `${BigInt(minLpTokens)}u128`       // min_lp: u128
    ];
    ...
    throw new Error(`Failed to create pool: ${error instanceof Error ? error.message : 'An unknown error occurred. Please try again or report it'}`);
  }
}
...
/**
 * Remove liquidity from a native ALEO + token pool
 * @param wallet - The user's wallet
 * @param publicKey - The user's public key
 * @param tokenId - ID of the custom token
 * @param lpTokensToBurn - Number of LP tokens to burn
 * @param minAleoOut - Minimum ALEO to receive (slippage protection)
 * @param minTokenOut - Minimum custom token to receive (slippage protection)
 * @returns Promise<boolean> - Success status
 */
export async function removeLiquidityNativeAleo(
  ...
  try {
    ...
    // Call the AMM DEX program
    // Parameters: [token_id: field, lp_tokens_to_burn: u128, min_aleo_out: u128, min_token_out: u128]
    const removeLiquidityInputs = [
      tokenId,                           // token_id: field
      `${BigInt(lpTokensToBurn)}u128`,   // lp_tokens_to_burn: u128
 ...
 * @param token2Id - ID of the second token
 * @param lpTokensToBurn - Number of LP tokens to burn
 * @param minToken1Out - Minimum first token to receive (slippage protection)
 * @param minToken2Out - Minimum second token to receive (slippage protection)
 * @returns Promise<boolean> - Success status
 */
export async function removeLiquidityTokens(
  ...
  try {
    ...
    // Call the AMM DEX program
    // Parameters: [token1_id: field, token2_id: field, lp_tokens_to_burn: u128, min_token1_out: u128, min_token2_out: u128]
    const removeLiquidityInputs = [
      token1Id,                           // token1_id: field
      token2Id,                           // token2_id: field
      `${BigInt(lpTokensToBurn)}u128`,    // lp_tokens_to_burn: u128
      `${BigInt(minToken1Out)}u128`,      // min_token1_out: u128
